...
import interfacedao.inventario.maestros.IF0006DAO;
...
public class SF0006 {
	...
	public List<F0006> buscarTodosOrdenadosPorDoc(String string, Double claveDoc) {
		List<String> lista = f4111DAO.findDisctincMcuByIldocAndIldct(claveDoc,
				string);
	...
	public List<F0006> buscarDondeExisteItem(Double idItem) {
		return f0006DAO.findByItmIn(idItem);
	}

	public List<String> buscarTodosDistinct() {
		List<F0006> f006 = f0006DAO.findAll();
		List<String> lista = new ArrayList<String>();
		for (Iterator<F0006> iterator = f006.iterator(); iterator.hasNext();) {
			F0006 string = (F0006) iterator.next();
			lista.add(string.getMcmcu());
		}
		return lista;
	}
